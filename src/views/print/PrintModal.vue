...
<template>
  <BasicModal @register="registerModal" title="打印" width="800px">
    <a-form name="basic" :label-col="{ span: 8 }" :wrapper-col="{ span: 16 }" autocomplete="off">
      ...
      <a-form-item v-if="formState.printType === 'direct'" label="打印机列表" name="printHost">
        <a-select placeholder="请选择打印机" v-model:value="formState.print">
          <a-select-option :value="null">请选择…</a-select-option>
          <template v-for="item in dictPrintList" :key="`${item.value}`">
            <a-select-option :value="item.id">
              <span style="display: inline-block; width: 100%" :title="item.template_name">
                {{ item.template_name }}
              </span>
...
<script setup>
  ...
  // import providers from './provider1.js';
  ...
  function connect(host) {
    ...
    // hiprint对象获取
    dictPrintList.value = hiprint.hiwebSocket.getPrinterList();
    console.log(dictPrintList.value);
  }
  /**
   * 构建设计器
